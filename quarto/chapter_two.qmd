...
All variables in the models below have been scaled and standardized to easily compare their effects. While there are some large and significant coefficients, the first item of note is that the R squared is quite low, meaning that not much of the variance has been explained. Additionally, while several of the coefficients point to an electorate of white, educated, culturally employed, liberal, newcomers, others, such as education and retail employment do not. Those could be genuine, the white transplant voting pool could be less educated than imagined or the EDs these candidates did well in could just be heterogenous (though the racial variables dispute this), but this nonetheless poses a puzzle.
...
lm4 <- lm(vote_share ~ log_income + nhb21p + h21p + white_transplant_ratio + cvap21bapp + hh21op + summer_noise_complaints + mean_noise, data = prog_cluster, na.action = na.omit)
lm5 <- lm(vote_share ~ log_income + nhb21p + h21p + white_transplant_ratio + cvap21bapp + hh21op + summer_noise_complaints + mean_noise + perc_retail, data = prog_cluster, na.action = na.omit)
